...
export async function GET(_: Request, ctx: RouteContext<"/api/rdos/[id]">) {
  ...
  const photos = await Promise.all(
    data.rdo_photos.map(async (photo: { path: string }) => {
  ...
  return Response.json({ ...data, rdo_photos: photos });
}

export async function PATCH(
  request: Request,
  ctx: RouteContext<"/api/rdos/[id]">,
) {
  const auth = await requireUser();
  if ("error" in auth) return auth.error;
  ...
  const { id } = await ctx.params;
  const { labor, equipment, activities, occurrences, project_id, ...rdo } =
    parsed.data;
  const { data: existing, error: lookupError } = await auth.supabase
    ...
    .single();
  if (lookupError)
    return Response.json({ error: lookupError.message }, { status: 404 });
  if (!(["DRAFT", "WITH_NOTES"] as string[]).includes(existing.status))
    return Response.json(
      { error: "Este RDO não pode mais ser editado." },
      { status: 409 },
    );
  const { data, error } = await auth.supabase
    .from("rdos")
    .update({ ...rdo, project_id })
    .eq("id", id)
    .select()
    .single();
  ...
  await Promise.all(
    childTables.map(async (table, index) => {
      await auth.supabase.from(table).delete().eq("rdo_id", id);
      if (inserts[index].length)
        await auth.supabase
          .from(table)
          .insert(inserts[index].map((row) => ({ ...row, rdo_id: id })));
    }),
  );
