...
export async function GET(_: Request, ctx: RouteContext<"/api/rdos/[id]/pdf">) {
  ...
  const { data: rdo, error } = await auth.supabase.from("rdos").select("*, projects(name, address, logo_path), rdo_labor(*), rdo_equipment(*), rdo_activities(*), rdo_occurrences(*)").eq("id", id).single();
  if (error) return Response.json({ error: error.message }, { status: 404 });
  const { data: logo } = rdo.projects.logo_path ? auth.supabase.storage.from("project-logos").getPublicUrl(rdo.projects.logo_path) : { data: null };
  rdo.projects.logo_url = logo?.publicUrl ?? null;
